...
{
	if (is_str_equal(line, "stack"))
		print_stack(KERNEL);
	else if (is_str_equal(line, "exec"))
		print_stack(KERNEL_EXEC);
	else if (is_str_equal(line, "clear"))
		clear_screen();
	else if (is_str_equal(line, "help"))
		help();
	else if (is_str_equal(line, "gdt"))
		print_stack(GDT);
	else if (is_str_equal(line, "idt"))
		print_stack(IDT);
	else if (is_str_equal(line, "halt")) {
		set_cpu_halted(true);
		while (get_cpu_halted())
			asm volatile("hlt");
	}
	else if (is_str_equal(line, "reboot"))
		outb(0x64, 0xFE);
	else
...
void shell(void)
{
	...
	while (1) {
		while (!get_cmd_ready())
			asm volatile("hlt"); // hlt for races conditions
